...
import static org.grizz.game.model.Direction.WEST;

@Component
public class WestMoveCommand extends CommandParser {
    @Autowired
    private MoveCommandExecutor commandExecutor;

    @Autowired
    public WestMoveCommand(Environment env) {
        super(env);
    }
    ...
    public PlayerResponse execute(String command, Player player, PlayerResponse response) {
        commandExecutor.move(WEST, player, response);
        return response;
    }
}
