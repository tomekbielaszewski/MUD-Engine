...
import static org.grizz.game.model.Direction.NORTH;

@Component
public class NorthMoveCommand extends CommandParser {
    @Autowired
    private MoveCommandExecutor commandExecutor;

    @Autowired
    public NorthMoveCommand(Environment env) {
        super(env);
    }
    ...
    public PlayerResponse execute(String command, Player player, PlayerResponse response) {
        commandExecutor.move(NORTH, player, response);
        return response;
    }
}
