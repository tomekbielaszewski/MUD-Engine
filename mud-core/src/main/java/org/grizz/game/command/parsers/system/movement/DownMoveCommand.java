...
import static org.grizz.game.model.Direction.DOWN;

@Component
public class DownMoveCommand extends CommandParser {
    @Autowired
    private MoveCommandExecutor commandExecutor;

    @Autowired
    public DownMoveCommand(Environment env) {
        super(env);
    }
    ...
    public PlayerResponse execute(String command, Player player, PlayerResponse response) {
        commandExecutor.move(DOWN, player, response);
        return response;
    }
}
