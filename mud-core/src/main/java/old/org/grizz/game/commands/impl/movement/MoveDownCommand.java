...
public class MoveDownCommand extends MovementCommand {
    @Autowired
    public MoveDownCommand(CommandUtils commandUtils, GameExceptionHandler exceptionHandler, MovementService movementService) {
    ...
    public PlayerResponse execute(final String command, final PlayerContext playerContext, PlayerResponse response) {
        return super.execute(Direction.DOWN, playerContext, response);
    }
}
